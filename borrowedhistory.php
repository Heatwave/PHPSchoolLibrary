		  ...
		  <table name="table" id="table" style="word-break:break-word" class="text-center table table-striped table-bordered table-condensed">
		    <thead>
				  <tr class="info">
						...
						<th class="text-center">归还日期</th>
					</tr>
				</thead>
				<tbody>
<?php
$query = 'SELECT borrowedbook.booksid,borrowedbook.isbn,books.name,books.author,borrowedbook.borroweddate,borrowedbook.returndate FROM borrowedbook,books WHERE books.isbn=borrowedbook.isbn AND borrowedbook.userid="'.$id.'" AND borrowedbook.returndate IS NOT NULL;';
$result = mysql_query($query,$db) or die (mysql_error($db));
